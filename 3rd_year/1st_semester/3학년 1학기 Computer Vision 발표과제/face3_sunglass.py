...
def sunglasses_function():
    ...
    # 이미지 불러오기
    image_glasses = cv2.imread('C:/Users/Adria/sunglasses1.png', cv2.IMREAD_UNCHANGED)
        ...
        while cap.isOpened():
            ...
            if results.detections:
                ...
                for detection in results.detections:
                    ...
                    h, w, _ = image.shape   # width, height channel : 이미지로부터 가로, 세로 크기 가져옴
                    nose_tip = (int(nose_tip.x * w) - 15, int(nose_tip.y * h) - 200)    # 이미지 내에서 실제 좌표 (x, y)

                    resized_image_glasses = cv2.resize(image_glasses[:, :, :4], (1000, 800))
                    resized_image_glasses_rgba = cv2.cvtColor(resized_image_glasses, cv2.COLOR_BGR2BGRA)
                    overlay(image, nose_tip[0], nose_tip[1], resized_image_glasses_rgba)
